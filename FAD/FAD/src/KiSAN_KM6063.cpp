...
KiSAN_KM6063::KiSAN_KM6063(unsigned int address, KiSANProtocol* kisanProtocol)
...
void KiSAN_KM6063::Initialize(HardwareSerial *hardwareSerial)
{
	_hardwareSerial = hardwareSerial;
}

bool KiSAN_KM6063::Process()
{
	bool result = false;
	int receiveResult = -1;
	switch(_state)
...
void KiSAN_KM6063::Send()
{
	...
	unsigned int outputState = 0x00;
	
	for(int i=0; i<KISAN_DIGITAL_OUTPUT_COUNT; i++)
	{
		if(global._kisanOutputs[i] == true)
		{
			outputState |= (1 << i);
		}
	}

	sprintf(sendMessage, "#02S0001,%04X", outputState);
